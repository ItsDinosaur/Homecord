...
export const initializeGlobalChatListeners = (initialMessages?: { channel_id: string, messages: Message[] }[]) => {
    ...
    if (initialMessages) {
        initialMessages.forEach(({ channel_id, messages }) => {
            globalChannelMessages.set(channel_id, messages);
        });
